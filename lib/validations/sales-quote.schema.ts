...
const requiredUuid = z.string().uuid("A valid selection is required");
...
export const SalesQuoteLineSchema = z
  .object({
    ...
    company_id: looseUuid,
    sales_quote_id: looseUuid,
    line_no: z.coerce.number().int().min(0).optional(),
    ...
    quantity_converted: z.coerce
      .number()
      .min(0, "Converted quantity cannot be negative")
      .optional()
      .nullable(),
    ...
    vat_product_posting_group_id: looseUuid,
  ...
  .superRefine((line, ctx) => {
    ...
    if (line.line_type === "GL_ACCOUNT" && !line.gl_account_id) {
      ctx.addIssue({
        code: "custom",
        message: "GL Account selection is required",
        path: ["gl_account_id"],
      });
    }
    ...
    const converted = line.quantity_converted ?? 0;

    if (converted > line.quantity) {
      ctx.addIssue({
        code: "custom",
        message: "Converted quantity cannot exceed quoted quantity",
        path: ["quantity_converted"],
      });
    }

    const remaining =
      line.quantity_remaining ?? Math.max(0, line.quantity - converted);

    if (remaining > line.quantity) {
      ctx.addIssue({
        code: "custom",
        message: "Remaining quantity cannot exceed quoted quantity",
        path: ["quantity_remaining"],
      });
    }

    if (line.discount_type === "PERCENT" && (line.discount_value ?? 0) > 100) {
      ctx.addIssue({
        code: "custom",
...
export const SalesQuoteSchema = z
  .object({
    ...
    customer_posting_group_id: looseUuid,
    vat_business_posting_group_id: looseUuid,
    ...
    customer_reference: looseString,

    source_of_quote: looseString,
    ...
    receivable_bank_id: looseUuid,
    receivable_bank: looseString,
    ...
    discount_amount: looseNumber,
    freight_charges: looseNumber,
    finance_charges: looseNumber,
    insurance_charges: looseNumber,
    vat_amount: looseNumber,
    ...
    conversion_status: z
      .enum(["NOT_CONVERTED", "PARTIALLY_CONVERTED", "FULLY_CONVERTED"])
      .optional(),
    ...
    accepted_at: looseString,
    accepted_by: looseUuid,

    rejected_at: looseString,
    rejected_by: looseUuid,
    rejection_reason: looseString,

    cancelled_at: looseString,
    cancelled_by: looseUuid,
    cancellation_reason: looseString,

    expired_at: looseString,

    converted_at: looseString,
    converted_by: looseUuid,
    ...
    terms_and_conditions: looseString,
    footer_text: looseString,
    reason: looseString,

    version: z.coerce.number().int().min(1).optional(),

    created_by: looseUuid,
    updated_by: looseUuid,
    created_at: looseString,
    updated_at: looseString,
    posted_at: looseString,
  })
  .superRefine((quote, ctx) => {
    if (quote.valid_from && quote.valid_until) {
      const from = new Date(quote.valid_from);
    ...
    if (quote.order_date && quote.valid_until) {
      ...
      if (
        !Number.isNaN(quoteDate.getTime()) &&
    ...
    if (quote.status === "accepted" && quote.is_rejected) {
      ctx.addIssue({
        code: "custom",
        message: "An accepted quote cannot be marked as rejected",
        path: ["status"],
      });
    }
    ...
    if (quote.status === "cancelled" && !quote.cancellation_reason) {
      ctx.addIssue({
        code: "custom",
...
export const SalesQuoteConversionRequestLineSchema = z.object({
  sales_quote_line_id: z.string().uuid("Quote line selection is required"),
...
export const SalesQuoteConversionRequestSchema = z.object({
  sales_quote_id: z.string().uuid("Sales quote selection is required"),

  lines: z
    .array(SalesQuoteConversionRequestLineSchema)
    .min(1, "At least one quote line must be selected for conversion"),
});

export const SalesQuoteConversionLineSchema = z.object({
  id: looseUuid,
  company_id: looseUuid,
  sales_quote_id: z.string().uuid(),
  sales_quote_line_id: z.string().uuid(),
  sales_order_id: z.string().uuid(),
  sales_order_line_id: z.string().uuid(),
  quantity_converted: z.coerce.number().positive(),
  converted_at: looseString,
  converted_by: looseUuid,
  created_at: looseString,
});

export const SalesQuoteOrderConversionSchema = z.object({
  id: looseUuid,
  company_id: looseUuid,

  sales_quote_id: z.string().uuid(),
  sales_order_id: z.string().uuid(),
  conversion_no: z.coerce.number().int().positive().optional(),
  conversion_date: looseString,
  converted_by: looseUuid,
  notes: looseString,
  created_at: looseString,
});

export const SalesQuotePayloadSchema = z.object({
  quote: SalesQuoteSchema,

  lines: z
    .array(SalesQuoteLineSchema)
    .min(1, "Sales quote must contain at least 1 line"),

  primary_address: SalesQuoteAddressSchema.nullable().optional(),
  billing_address: SalesQuoteAddressSchema.nullable().optional(),
  shipping_address: SalesQuoteAddressSchema.nullable().optional(),

  allow_empty_lines: z.boolean().optional(),
});

export type SalesQuoteInput = z.infer<typeof SalesQuoteSchema>;
export type SalesQuoteAddressInput = z.infer<typeof SalesQuoteAddressSchema>;
export type SalesQuoteLineInput = z.infer<typeof SalesQuoteLineSchema>;
export type SalesQuotePayloadInput = z.infer<typeof SalesQuotePayloadSchema>;
export type SalesQuoteConversionRequestInput = z.infer<
  typeof SalesQuoteConversionRequestSchema
>;
export type SalesQuoteConversionLineInput = z.infer<
  typeof SalesQuoteConversionLineSchema
>;
export type SalesQuoteOrderConversionInput = z.infer<
