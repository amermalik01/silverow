...
export class AllocationService {
  ...
  static async applyAP(
    ...
    for (const alloc of allocations) {
      ...
      const allocInsertRes = await client.query(
        ...
        ) VALUES ($1, 'AP', $2, $3, $4, $5, $6, CURRENT_DATE, $7, $8)
         RETURNING id`,
        [
          companyId,
          paymentLedgerId,
  ...
  private static async postFxGlEntries(
    ...
    // 2. Determine Debits and Credits
    let controlDebit = 0;
    let controlCredit = 0;
    let fxDebit = 0;
    let fxCredit = 0;
    ...
    const query = `INSERT INTO gl_ledger_entries (
      company_id,
      account_id,
      transaction_id,
      entry_no,
    ...
    // 3. Get next transaction ID sequence
    const txKeyResult = await client.query(
      "SELECT nextval('gl_transaction_id_seq') AS tx_id",
    );
    const transactionId = parseInt(txKeyResult.rows[0].tx_id, 10);
    ...
    await client.query(query, [
      companyId, // $1
      controlGlAccountId, // $2
      transactionId, // $3
      documentNo, // $4
      "FX_VARIANCE", // $5
      refTag, // $6
      `Realized FX ${isGain ? "Gain" : "Loss"} Allocation Adjustment`, // $7
      controlDebit, // $8
      controlCredit, // $9
      currencyId || null, // $10
      1.0, // $11 (exchange_rate)
      0.0, // $12 (debit_fcy)
      0.0, // $13 (credit_fcy)
    ...
    const txKeyResult2 = await client.query(
      "SELECT nextval('gl_transaction_id_seq') AS tx_id",
    );
    const transactionId2 = parseInt(txKeyResult2.rows[0].tx_id, 10);
    ...
    await client.query(query, [
      ...
      "FX_VARIANCE", // $5
      refTag, // $6
      `Realized FX ${isGain ? "Gain" : "Loss"} Line`, // $7
      fxDebit, // $8
      fxCredit, // $9
      currencyId || null, // $10
      1.0, // $11 (exchange_rate)
