...
export class PurchaseReceiptService {
  ...
  static async createTransactional(
    ...
    // Extract your current create logic code block here but omit the client.connect(),
    // BEGIN, COMMIT, ROLLBACK, and client.release() statements so it embeds smoothly
    // directly into the main API controller's transactional lifecycle execution chain.
    // (Use the passed `client` argument instead of pool.connect()).
  }
