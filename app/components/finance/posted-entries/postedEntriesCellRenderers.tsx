...
export const postedEntriesCellRenderers: Record<string, (row: PostedLedgerEntry) => React.ReactNode> = {
  posting_date: (row) => formatDate(row.posting_date),
  document_type: (row) => row.document_type || "—",
  document_number: (row) => <span className="font-semibold">{row.document_number || "—"}</span>,
  ...
  debit: (row) => formatAmount(row.debit_fcy),
  credit: (row) => formatAmount(row.credit_fcy),
  amount: (row) => formatAmount(row.net_amount_fcy),
  debit_lcy: (row) => formatAmount(row.debit_lcy),
  credit_lcy: (row) => formatAmount(row.credit_lcy),
  amount_lcy: (row) => formatAmount(row.net_amount_lcy),
