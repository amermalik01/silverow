// app/components/purchases/posted-debit-notes/PostedTransactionsModal.tsx
...
> = ({ isOpen, onClose, debitNoteId, debitNoteNo }) => {
  ...
  useEffect(() => {
    if (!isOpen || !debitNoteId) return;

    async function fetchLedgerEntries() {
      try {
        setLoading(true);
        const res = await fetch(
          `/api/posted-debit-notes/${debitNoteId}/posted-entries`,
        );
      ...
      <div className="bg-white dark:bg-slate-900 rounded-lg shadow-xl w-full max-w-6xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh]">
        ...
        <div className="bg-[#1b431c] text-white px-6 py-3 flex items-center justify-between">
          <h2 className="text-sm font-bold tracking-wide text-white">
            Accounting Entries for Debit Note No. {debitNoteNo || "Draft"}
          </h2>
          <button
            type="button"
            onClick={onClose}
                    ...
                    filteredEntries.map((row, idx) => (
                      ...
                      >
                        <td className="p-2.5 whitespace-nowrap">
                          {row.posting_date
                            ? format(new Date(row.posting_date), "dd/MM/yyyy")
                            : "—"}
                        </td>
                        <td className="p-2.5">
                        ...
                        <td className="p-2.5 font-semibold">
                          {row.document_number || debitNoteNo}
                        </td>
