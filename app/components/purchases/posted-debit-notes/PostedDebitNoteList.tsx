...
export default function PostedDebitNoteList({ slug }: Props) {
  ...
  const [status, setStatus] = useState("POSTED"); // Hardcoded to track Posted context by default
      ...
      <div className="bg-white dark:bg-slate-900 border dark:border-slate-800 rounded-xl p-4 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          >
            ...
            <option value="">All Ledger Records</option>
          </select>
        </div>
              ...
              {!data.length ? (
                ...
                data.map((row) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      <PostedDebitNoteStatusBadge status={row.status} />
                    </td>
