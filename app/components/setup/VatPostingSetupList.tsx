...
export default function VATPostingSetupPage() {
  ...
  const [prodOptions, setProdOptions] = useState<{ value: string; label: string }[]>([]);
    ...
    async function loadDropdownData() {
      try {
        ...
        if (!bizRes.ok || !prodRes.ok) {
          throw new Error("Failed to load dependency relations for posting setup matrix.");
        }
  ...
  if (loadingOptions) {
    return <div className="p-6 text-gray-500">Loading master group dependencies...</div>;
  }
  ...
  const dynamicFields: Field[] = [
    ...
    { name: "sales_vat_account_id", label: "Sales VAT Account (UUID)", type: "text", required: false },
    { name: "purchase_vat_account_id", label: "Purchase VAT Account (UUID)", type: "text", required: false },
  ];
