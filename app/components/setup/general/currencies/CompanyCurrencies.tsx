...
/* export default function CompanyCurrencies() {
  ...
  return (
    <div>
      ...
      <h2 className="text-lg font-bold mb-3">Company Currencies</h2>
        ...
        <tbody>
          {data.map((c) => (
            <tr key={c.id}>
              <td>{c.code}</td>
              <td>{c.name}</td>
              <td>{formatCurrency(c.exchange_rate, c.code)}</td>
              <td>{c.is_base ? "Yes" : "No"}</td>
              <td className="flex gap-2">
                {!c.is_base && (
                  <button
                    onClick={async () => {
                      await fetch("/api/setup/general/company/currencies/set-base", {
                        method: "POST",
                        body: JSON.stringify({ currency_id: c.id }),
                      });
                      fetchData();
                ...
                {!c.is_base && (
                  <button
                    onClick={async () => {
                      ...
                      await fetch(`/api/setup/general/company/currencies/${c.id}`, {
                        method: "DELETE",
                      });
