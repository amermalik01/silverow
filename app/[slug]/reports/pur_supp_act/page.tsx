...
export default function SupplierActivityReport() {
  ...
  const exportToCSV = () => {
    ...
    const rows = reportData.map((row) => [
      ...
      row.currency_code !== "GBP" ? row.remaining_amount_fcy : "",
      row.amount_lcy,
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-xs items-center">
          ...
          <div className="flex flex-col gap-1">
            ...
            >
              ...
              <option value="Journals">Journals & FX Adjustments</option>
            </select>
          </div>
              ...
              {loading ? (
                ...
                reportData.map((row) => (
                  ...
                  >
                    ...
                    {/* LCY Column Displays */}
                    <td
                      className={`p-3 text-right font-bold tabular-nums ${row.amount_lcy < 0 ? "text-red-600" : "text-slate-900"}`}
                    >
                      {formatCurrency(row.amount_lcy)}
                    </td>
                    <td
                      className={`p-3 text-right font-bold tabular-nums ${row.remaining_amount_lcy < 0 ? "text-red-600" : "text-emerald-800"}`}
                    >
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 bg-white border border-slate-200 px-4 py-2.5 rounded shadow-sm text-right">
              <div>
                ...
                <span className="text-slate-900 font-bold font-mono ml-1">
                  {formatCurrency(
                    reportData.reduce(
                      (acc, curr) => acc + Number(curr.original_amount_fcy),
                      0,
                    ),
                  )}
                </span>
              ...
              <div className="border-t sm:border-t-0 sm:border-l border-slate-200 pt-2 sm:pt-0 sm:pl-8">
                ...
                <span className="text-emerald-800 font-bold font-mono ml-1">
                  {formatCurrency(
                    reportData.reduce(
                      (acc, curr) => acc + Number(curr.remaining_amount_fcy),
                      0,
                    ),
                  )}
                </span>
