// app/[slug]/reports/sales_stmt/page.tsx
...
export default function CustomerStatementReport() {
  ...
  const [statements, setStatements] = useState<CustomerStatementGroup[]>([]);
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed fetching customer statements");
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, ws, "Customer Statements");
    XLSX.writeFile(
      workbook,
      `Customer_Statement_${format(new Date(), "yyyyMMdd")}.xlsx`,
    );
        ...
        <div className="p-6 space-y-12">
          {loading ? (
            <div className="p-12 text-center font-sans text-xs text-slate-400 italic">
              Generating customer statement matrix...
            </div>
          ) : statements.length === 0 ? (
            <div className="p-12 text-center font-sans text-xs text-slate-400 italic">
              No open customer statements available as at {formatDate(asOfDate)}
              .
            </div>
          ) : (
            statements.map((stmt) => (
                  ...
                  <table className="w-full text-left border-collapse text-xs">
                    <thead className="bg-[#0b3310] text-white font-bold">
                      <tr>
                        ...
                        <th className="p-2.5">Customer Ref. No.</th>
