// app/[slug]/reports/sales_ageing/page.tsx
...
export default function CustomerAgeingReport() {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/reports/customer-ageing?${params.toString()}`,
      );
  ...
  const exportToExcel = () => {
    ...
    if (viewMode === "Summary") {
      ...
      detailedData.forEach((group) => {
        group.entries.forEach((e) => {
          rows.push({
            Customer: `${group.vendor_name} (${group.vendor_no})`,
    ...
    XLSX.writeFile(
      workbook,
      `Customer_Ageing_${viewMode}_${format(new Date(), "yyyyMMdd")}.xlsx`,
    );
        ...
        <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-lg font-bold text-slate-900 tracking-tight flex items-center gap-2">
              <FileText className="text-emerald-700 h-5 w-5" /> Customer Ageing
              Report ({viewMode})
            </h1>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td
                      ...
                      className="p-12 text-center font-sans text-xs text-slate-400 italic"
                    >
                      No customer ageing balances found as at{" "}
                      {formatDate(asOfDate)}.
                    </td>
                  </tr>
                ) : (
