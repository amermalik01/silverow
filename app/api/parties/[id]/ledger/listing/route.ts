...
export async function POST(
  ...
  try {
    ...
    const targetPartyType = isSupplier ? "SUPPLIER" : "CUSTOMER";

    // SQL Expression for sub-ledger amount signage based on Party Role
    const signedFcyOriginalExpr = isSupplier
      ? `CASE 
    ...
    const signedFcyRemainingExpr = isSupplier
      ? `CASE 
          WHEN UPPER(e.document_type) LIKE '%PAYMENT%' 
            OR UPPER(e.document_type) LIKE '%DEBIT_NOTE%' 
            OR UPPER(e.document_type) LIKE '%REFUND%' 
    ...
    const signedLcyOriginalExpr = isSupplier
      ? `CASE 
          WHEN UPPER(e.document_type) LIKE '%PAYMENT%' 
            OR UPPER(e.document_type) LIKE '%DEBIT_NOTE%' 
            OR UPPER(e.document_type) LIKE '%REFUND%' 
    ...
    const signedLcyRemainingExpr = isSupplier
      ? `CASE 
          WHEN UPPER(e.document_type) LIKE '%PAYMENT%' 
            OR UPPER(e.document_type) LIKE '%DEBIT_NOTE%' 
            OR UPPER(e.document_type) LIKE '%REFUND%' 
    ...
    // 1. Party Summary calculation query
    const summaryQuery = `
      ...
      fx_totals AS (
        ...
        WHERE gle.company_id = $1
          ...
          AND UPPER(gle.party_type::text) = '${targetPartyType}'
          AND gle.party_id = $2
      )
...
export async function POST(
  ...
  try {
    ...
    const summaryQuery = `
      WITH party_totals AS (
        SELECT
          COALESCE(SUM(${signedFcyOriginalExpr}), 0) AS total_original_fcy,
          COALESCE(SUM(${signedFcyRemainingExpr}), 0) AS total_remaining_fcy,
          COALESCE(SUM(${signedLcyOriginalExpr}), 0) AS total_original_lcy,
          COALESCE(SUM(${signedLcyRemainingExpr}), 0) AS total_remaining_lcy,
          COUNT(
            CASE
              WHEN e.is_open = true AND ABS(e.remaining_amount_fcy) > 0
              THEN 1
      ...
      fx_totals AS (
        ...
        WHERE gle.company_id = $1
          AND gle.source_type::text = 'FX_VARIANCE'
          AND LOWER(gle.party_type::text) = LOWER('${glPartyType}')
          AND gle.party_id = $2
      )
      SELECT
        p.total_original_fcy,
