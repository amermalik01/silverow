// app/api/sales/sales-orders/customers/[customerId]/locations/route.ts
...
export async function GET(
  ...
  try {
    ...
    // Verify customer belongs to company and is a valid customer
    const customerCheck = await pool.query(
      `SELECT id FROM parties WHERE id = $1 AND company_id = $2 AND is_customer = true`,
    ...
    // Query all shipping or primary addresses associated with the Customer
    const queryText = `
      SELECT 
        pa.id,
    ...
    console.error("Error fetching customer delivery locations:", err);
    return NextResponse.json(
