...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!itemId && !debitNoteLineId) {
      return NextResponse.json(
        { success: false, error: "item_id or debit_note_line_id is required." },
        { status: 400 },
      );
    }
    ...
    if (debitNoteLineId) {
      const existingQuery = `
        SELECT
          ...
          COALESCE(
            (
              ...
              INNER JOIN debit_note_lines rdl ON rdl.id = r.debit_note_line_id
              INNER JOIN debit_notes rdn ON rdn.id = rdl.debit_note_id
        ...
        INNER JOIN inventory_allocations source
          ON source.id = ia.source_allocation_id AND source.company_id = ia.company_id
        INNER JOIN debit_note_lines current_dnl
          ON current_dnl.id = ia.debit_note_line_id AND current_dnl.company_id = ia.company_id
        INNER JOIN debit_notes rdn_current
          ON rdn_current.id = current_dnl.debit_note_id AND rdn_current.company_id = ia.company_id
      ...
      // console.log("========== PARAMS ==========");
      // console.log("debitNoteLineId ==== ", debitNoteLineId);
      // console.log("==========================");
      ...
      const data = existingResult.rows.map((row) => {
        ...
        return {
          ...
          available_quantity: availableBeforeThisDn,
        };
      });
    ...
    // Unallocated Stock Fetching
    const queryParamsList: unknown[] = [companyId, itemId];
    let whereClause = `
      ia.company_id = $1
      AND ia.item_id = $2
    ...
    if (purchaseOrderLineId) {
      queryParamsList.push(purchaseOrderLineId);
      whereClause += ` AND ia.purchase_order_line_id = $${queryParamsList.length}`;
    } else if (purchaseInvoiceLineId) {
      queryParamsList.push(purchaseInvoiceLineId);
      whereClause += ` AND ia.purchase_invoice_line_id = $${queryParamsList.length}`;
    }
