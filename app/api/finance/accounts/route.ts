...
export async function GET() {
  ...
  try {
    ...
    const accounts = formatted.map((account) => {
      if (account.gl_account_type === "End Total" && account.range_start_code && account.range_end_code) {
        const rangeAccounts = formatted.filter(
          (a) => a.gl_account_type === "Posting" && a.code >= account.range_start_code! && a.code <= account.range_end_code!
        );
        return {
          ...account,
          display_debit: rangeAccounts.reduce((sum, a) => sum + a.display_debit, 0),
          display_credit: rangeAccounts.reduce((sum, a) => sum + a.display_credit, 0),
        };
      }
      return account;
    });
    ...
    return NextResponse.json({ error: "Failed tracking structural chart matrices" }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: dbError.message || "Engine transactional breakdown." }, { status: 422 });
  }
}
