...
interface CustomerStatementGroup {
  customer_id: string;
  vendor_no: string;
  vendor_name: string;
  country: string;
  ...
  try {
    ...
    if (customerIdsParam) {
      ...
      if (customerIds.length > 0) {
        ...
        whereConditions.push(`e.customer_id = ANY($${queryParams.length}::uuid[])`);
      }
    }
    ...
    // 2. Fetch Customer Statement Ledger Entries & Match Assigned Payable Bank
    ...
    const groupedStatements: Record<string, CustomerStatementGroup> = {};
    ...
    // 3. Compute Ageing Summary Matrix per Customer
    const ageingQuery = `
      SELECT 
        p.id AS customer_id,
        COALESCE(c.code, 'GBP') AS currency_code,
    ...
    ageingResult.rows.forEach((ar) => {
      const vendorKey = ar.customer_id;
      if (groupedStatements[vendorKey]) {
    ...
    console.error(err);
    return NextResponse.json(
      { error: "Failed to generate Customer Statement Report" },
      { status: 500 }
    );
  }
}